...
function ConfigTool({ dispatch }) {
  //const isMobile = useMatchMedia('(min-width: 320px) and (max-width: 767px)');
  const { currentUser } = useLoaderData();
  const [hide, togglehide] = useState(true);

  function downloadAsImage(e) {
    e.stopPropagation();
    dispatch({
      type: UPDATE_MODAL_TYPE,
      payload: {
        modalType: 'downloadImage'
      }
    })
  }

  function onDeleteCanvas(e) {
    e.stopPropagation();
    dispatch({
      type: UPDATE_MODAL_TYPE,
      payload: {
        modalType: 'deleteCanvas'
      }
    })
  }

  function onShareLink(e) {
    e.stopPropagation();
    dispatch({
      type: UPDATE_MODAL_TYPE,
      payload: {
  ...
  return (
    <div className="configTool">
      ...
      <div className={`wrapper ${hide ? 'hide-tools' : ''}`} >
        <div className='icon' onClick={onDeleteCanvas}>
        ...
        <div className='icon' onClick={downloadAsImage}>
        ...
        {currentUser ? (<div className='icon' onClick={onShareLink}>
        ...
        <div className='icon' onClick={togglePreferences} data-testid="settings-icon">
